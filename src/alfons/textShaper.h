...
public:
    ...
    LineLayout shape(std::shared_ptr<Font>& font, const std::string& text,
        ...
        return shape(font, text,
                                hb_language_from_string(langHint.c_str(), -1),
                                overallDirection);
    }

protected:

    LineLayout shape(std::shared_ptr<Font>& font, const TextLine& line,
                                const std::vector<TextRun>& range);

    LineLayout shape(std::shared_ptr<Font>& font, TextLine& line);

    bool processRun(const FontFace& face, const TextRun& run);
